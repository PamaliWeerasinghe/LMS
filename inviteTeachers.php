...
$t=$_POST["tid"];
$email=Database::search("SELECT * FROM `teacher` WHERE `id`='".$t."'");
    ...
    $password=uniqid();
    Database::iud("UPDATE `teacher` SET `verification_code`='".$verification_code."' WHERE `id`='".$t."'");
    Database::iud("UPDATE `teacher` SET `password`='".$password."' WHERE `id`='".$t."'");
    Database::iud("UPDATE `teacher` SET `username`='".$email2[0]."' WHERE `id`='".$t."'");
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Teacher Registeration | Online Student Management System';
    ...
    if (!$mail->send()) {
        echo 'Verification code sending failed to '.$email2[0].'<br/>';
    } else {
